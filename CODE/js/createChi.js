...
function createChi(nCont, R) {
	...
	if (isString(n)) {
		...
		let oidCont = nCont.oid;
		let oCont = R.getO(oidCont);

		let oids = calcContentFromData(oidCont,oCont, n, R);

		// console.log('string case olist=',owner,n,olist);
		//if owner does not have corresponding property, nothing is created!
		if (!oids) {
			console.log(oidCont, 'does not have', n, 'or prop', n, 'of', oidCont, 'is not a list!!!')
			return [];
		}
		if (!isList(oids)) oids=[oids];
	...
	// eg. panels: [1,2,3]
		...
		for (const x of n) {
			...
			if (isdef(x.data) && isdef(nCont.oid) && nundef(n1.oid)) {
				n1.oid = nCont.oid;
				content = calcContent(n1.oid,R.getO(n1.oid), n1.data);
	...
	//case 2: wenn n ein oid besitzt muss fuer dieses 1 child gemacht werden
		...
		//console.log('...case 2')
		let n1 = jsCopy(n);
		n1.content = n.data ? calcContent(n1.oid,R.getO(n1.oid), n.data) : null;
		createLC(n1, nCont.uid, R);
		chNodes.push(n1);
	}
	//case 3: wenn n ein data besitzt muss fuer dieses 1 child gemacht werden
		...
		let n1 = jsCopy(n);
		n1.oid = nCont.oid;
		n1.content = calcContent(n1.oid,R.getO(n1.oid), n.data);
		createLC(n1, nCont.uid, R);
		chNodes.push(n1);
	}
	...
	//case 5: n.data, n NOT container, nCont.pool + nCont.data, NO nCont.oid, 
		...
		let data = calcContent(nCont.oid,R.getO(nCont.oid), nCont.data);
